...
// The picker state lives in the URL path (/timetable/t/{term}/p/{program}/s/{section}).
// We read the term and program ids from the searchable comboboxes and the section ids
// from the native select, then drive the cascade by navigating the path form directly.
// That keeps the picker exercised while being deterministic (no soft-navigation click
// races). Lands the page on the first section that renders a timetable.
async function firstTermId(page: Page): Promise<string> {
...
async function programIds(page: Page, term: string): Promise<string[]> {
...
async function sectionIds(page: Page, term: string, program: string): Promise<string[]> {
...
async function cascadeToPopulatedSection(
  page: Page,
