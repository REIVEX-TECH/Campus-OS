...
const bodySchema = z.discriminatedUnion('action', [
  ...
  z.object({
    tenant: z.string().min(1).max(64),
    action: z.literal('ephemerality'),
    value: z.enum(EPHEMERALITY),
  }),
]);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (gate.data.action === 'read') {
    const res = await markRead(
      ...
      gate.tenant.slug,
      id,
      gate.settings.afterViewingGraceSeconds,
    );
    if (!res.ok) return refusalResponse(res.error);
    return Response.json({ ok: true });
  }
  if (gate.data.action === 'ephemerality') {
    const res = await setEphemerality(gate.actor, gate.tenant.slug, id, gate.data.value);
    if (!res.ok) return refusalResponse(res.error);
    return Response.json({ ok: true });
  }
