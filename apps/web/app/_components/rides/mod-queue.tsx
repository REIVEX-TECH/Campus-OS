...
export interface RidesModEntry {
  reportId: string;
...
export interface RidesModLabels {
  empty: string;
  targetRide: string;
  targetUser: string;
  reportedBy: string;
...
/** The rides moderation queue: open reports with remove (rides) / dismiss. */
export function RidesModQueue({
  base,
  tenant,
  entries,
  labels,
  ...
  base: string;
  tenant: string;
  entries: RidesModEntry[];
  labels: RidesModLabels;
}) {
  ...
  return (
    <ul className="flex flex-col gap-2">
      {entries.map((e) => {
        const isRide = e.targetType === 'ride_post';
        return (
          <li key={e.reportId} className="ios-card flex flex-col gap-2 rounded-2xl p-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-[11px] font-medium text-destructive">
                {isRide ? labels.targetRide : labels.targetUser}
              </span>
              <span className="text-[11px] text-muted-foreground">{e.reason}</span>
            </div>
            {isRide && (e.rideOrigin || e.rideDest) ? (
              <p className="text-sm font-medium">
                {e.rideOrigin ?? '?'} {'→'} {e.rideDest ?? '?'}
              </p>
            ) : null}
            {!isRide && e.reportedHandle ? (
              <p className="text-sm font-medium">@{e.reportedHandle}</p>
            ) : null}
            {e.note ? <p className="text-sm text-muted-foreground">{e.note}</p> : null}
            <p className="text-[11px] text-muted-foreground">
              {labels.reportedBy.replace('{who}', e.reporterHandle ?? '?')}
            </p>
            {removing === e.reportId ? (
              <form
