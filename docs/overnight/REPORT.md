...
| #27 admin analytics                  | pending   | Read-only `/admin/analytics` behind the existing admin gate: at-a-glance totals, teacher/room coverage bars, classes by kind and by ISO weekday, pending-review counts. New `TimetableQueries.analytics()` (aggregate counts only, no new data), SSR CSS bars (no chart lib). Integration + e2e.         |
