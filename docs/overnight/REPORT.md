...
| #25 module stubs                     | pending   | Tenant home becomes a module hub: 8 module cards (3 live → their pages, 5 "Coming soon" stubs). New `lib/modules.ts` catalogue + `soonModule` validator, `/soon/[module]` stub page (notFound for live/unknown keys), i18n for all module labels/descriptions, e2e. Pure UI, no auth, no feature logic.  |
