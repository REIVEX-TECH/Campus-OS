...
| #33 admin noindex                    | pending   | Add an admin `layout.tsx` setting `robots: noindex, nofollow` for the whole admin subtree, so the publicly reachable login (and everything behind it) is never indexed. Public pages are unaffected. e2e asserts the login carries `noindex`.                                                                                                                                                                                   |
