...
| #24 search                           | pending   | Public read-only search: teacher to their week, course to a "where/when/who" page. Debounced client box writing `?q`, `searchTeachers`/`searchCourses`/`getCourse`/`courseTimetable`, new `/courses/[id]` page, nav item. Full-width, iOS, both themes.                                                  |
